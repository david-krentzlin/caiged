...
func TestNormalizeOptionsNoMountGHWins(t *testing.T) {
	opts := RunOptions{MountGHRW: true, NoMountGH: true}
...
func TestNormalizeOptionsMountGHRWEnablesMountGH(t *testing.T) {
	opts := RunOptions{MountGHRW: true}
	normalized := normalizeOptions(opts)
...
func TestNormalizeOptionsNoMountOpenCodeAuthWins(t *testing.T) {
	opts := RunOptions{NoMountOpenCodeAuth: true}
	normalized := normalizeOptions(opts)
...
func TestNormalizeOptionsDefaultsToMount(t *testing.T) {
	opts := RunOptions{}
	normalized := normalizeOptions(opts)

	if !normalized.MountGH {
		t.Fatalf("expected MountGH to be true by default")
	}
	if !normalized.MountOpenCodeAuth {
		t.Fatalf("expected MountOpenCodeAuth to be true by default")
	}
}
